...
import {getEduTopics} from './store/action';
...
@connect (
  state => {
    return state.educationDetailsReducer;
  },
  {getEduTopics}
)
class EduTopicAdd extends Component {
  ...
  componentDidMount () {
    ...
    this.props.getEduTopics (params);
  }
  componentWillUpdate () {
  ...
  componentWillReceiveProps (nextProps) {
    const {listSingle, listMultiple, listReading} = nextProps;
  ...
  render () {
    const {listSingle, listMultiple, listReading, answerType} = this.state;
            ...
            <FormItem {...formItemLayout} label="测试类型" hasFeedback>
              ...
              >
                ...
                <TabPane tab="单选题" key="listSingle">
                  <Collapse accordion>
                    {this.itemRender (listSingle)}
                  </Collapse>
                </TabPane>
                <TabPane tab="多选题" key="listMultiple">
                  <Collapse accordion>
                    {this.itemRender (listMultiple)}
                  </Collapse>
                </TabPane>
                <TabPane tab="阅读题" key="listReading">
                  <Collapse accordion>
                    {this.itemRender (listReading)}
                  </Collapse>
                </TabPane>
              </Tabs>
            </FormItem>
  ...
  joinTopic = (data, i, e) => {
    ...
    this.setState (prevState => ({
      [answerType]: prevState[answerType].map (v => {
        v.anserId === data.anserId ? (v.ischecked = !v.ischecked) : null;
        return v;
        // let isExist = itemId.some(item=>{
        //     if(item.anserId===data.anserId){
        //        return true;
        //    } else{
        //        return false;
        //    }
        ...
        //         key: '1',
        //         message: '提示',
        //         description: '已加入题库，不可重复添加',
        //     });

        // }else{
        //     v.anserId === data.anserId ? (v.ischecked = e.target.checked) : null;
        //     return v;
        // }
      }),
    }));
  ...
  answerType = e => {
    ...
    this.state[e].length === 0 && this.props.getEduTopics (params);
  };
}
